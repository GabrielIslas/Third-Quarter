## -------------------------------------------------------------
## Stat/Q Sci 403 Lab 8 | Spring 2017 | University of Washington
## -------------------------------------------------------------
###                           ###
...
plot(dat_kde, lwd=3, ylim=c(0, 0.04), col="blue")
...
####     density histogram.
{datex = iris$Petal.Width
datex_kde= density(datex, bw=0.2)
hist(datex, col="skyblue", probability = T)
lines(datex_kde, lwd=3, col="blue")
}
#### (2) Use h=0.2, change the smoothing kernel to 'triangular' and 
####     'rectangular'. Show the estimated density curves and compare to
####     the histogram
{hist(datex, col="skyblue", probability = T)
lines(density(datex, bw=0.2, kernel = "triangular"), lwd=3, col="brown")
lines(density(datex, bw=0.2, kernel = "rectangular"), lwd=3, col="purple")
legend("topright",c("triangular","rectangular"), col=c("brown","purple"),
       lwd=6)
}
#### (3) In a single plot, show the density curves estimated by KDE with
####     h = 0.1, 0.2, 0.5. 
{plot(density(datex, bw=0.1), col="limegreen", lwd=4, ylim=c(0,1),
     xlab="Petal.Width", main="Bandwidth Comparison")
lines(density(datex, bw=0.2), lwd=4, col="orange")
lines(density(datex, bw=0.5), lwd=4, col="orchid")
legend("topright",c("h=0.1","h=0.2","h=0.5"), 
       col=c("limegreen","orange","orchid"),
       lwd=6)
}
...
{h0 = 0.2
datex_kde = density(datex, from = -0.5, to=3, bw=h0)
n = length(datex)

B = 10000
gr_y = matrix(NA, nrow=B, ncol=length(datex_kde$x))
for(i in 1:B){
  w = sample(n,n,replace=T)
  datex_BT = datex[w]
  datex_kde_BT = density(datex_BT, from = -0.5, to=3, bw=h0)
  gr_y[i,] = datex_kde_BT$y
}
gr_y_sd = sqrt( diag(var(gr_y)))

plot(datex_kde, ylim=c(0,1.2),lwd=3, main="h0 = 0.2")
kde_upper = datex_kde$y+qnorm(0.95)*gr_y_sd
kde_lower = datex_kde$y-qnorm(0.95)*gr_y_sd
polygon(x=c(datex_kde$x, rev(datex_kde$x)),
        y=c(kde_upper, rev(kde_lower)),
        col='gray', border = 'gray')
lines(datex_kde,lwd=3)
}
...
plot(datex_kde, ylim=c(0,1.2),lwd=3, main="h0 = 0.1", col="brown")
kde_upper = datex_kde$y+qnorm(0.95)*gr_y_sd
kde_lower = datex_kde$y-qnorm(0.95)*gr_y_sd
polygon(x=c(datex_kde$x, rev(datex_kde$x)),
        y=c(kde_upper, rev(kde_lower)),
        col='tan', border = 'tan')
lines(datex_kde,lwd=3, col="brown")
}
...
plot(datex_kde, ylim=c(0,1.2),lwd=3, main="h0 = 0.5", col="purple")
kde_upper = datex_kde$y+qnorm(0.95)*gr_y_sd
kde_lower = datex_kde$y-qnorm(0.95)*gr_y_sd
polygon(x=c(datex_kde$x, rev(datex_kde$x)),
        y=c(kde_upper, rev(kde_lower)),
        col='plum', border = 'plum')
lines(datex_kde,lwd=3, col="purple")
}
...
plot(x=dat1_kde$x, y=true_density, ylim=c(0,0.20), type="l",
     lwd=3, col="blue")
...
{U = rmultinom(1, n, prob=c(0.35,0.65))
mu1 = 0
mu2 = 4
sd1 = sd2 = 1
dat1 = c(rnorm(U[1], mean=mu1,sd=sd1), rnorm(U[2], mean=mu2,sd=sd2))
...
{dat1_kde = density(dat1, from=-5, to=10, n=1000,bw=h0)
true_density = 0.35*dnorm(dat1_kde$x,mean=mu1,sd=sd1)+
  0.65*dnorm(dat1_kde$x,mean=mu2,sd=sd2)
plot(x=dat1_kde$x, y=true_density, ylim=c(0,0.35), type="l",
     lwd=3, col="blue")
...
for(i in 1:N){
  U = rmultinom(1, n, prob=c(0.35,0.65))
  dat1_MC = c(rnorm(U[1], mean=mu1,sd=sd1), rnorm(U[2], mean=mu2,sd=sd2))
  dat1_kde_MC = density(dat1_MC, from=-5, to=10, n=1000,bw =h0)
  dat1_kdey_MC[i,] = dat1_kde_MC$y
}
dat1_kdey_mse = colSums((t(t(dat1_kdey_MC)- true_density))^2)/N
...
sum(dat1_kdey_mse)*(dat1_kde$x[2]-dat1_kde$x[1])
  # the true MISE (after scaling): the latter quantity is the bin size  
}
#### (4) Change the smoothing bandwidth a bit to analyze how the MISE
####     changes with respect to the smoothing bandwidth.
####     For simplicity, we consider the smoothing bandwidth within [0.1,0.5].
...
plot(x= h0_seq, y= mise_seq, type="l", ylim=c(0,0.2),lwd=3,
     ylab="MISE", xlab="smoothing bandwidth", col="orchid")
}
###                ###
### Part 4: 2D KDE ###
###                ###
...
colP = colorRampPalette(c("white","green","red"))
image(x=iris_kde$x1,y=iris_kde$x2,
      z=iris_kde$fhat, xlab="Sepal.Length", 
      ylab="Petal.Length", main="Density (Heat Map)",
      col=colP(50))
...
### colored contour
col_tmp <- colorRampPalette(c("white","dodgerblue","red"))(10)
...
{X_kde_2D = bkde2D(data_new, bandwidth = 0.2)
contour(x=X_kde_2D$x1, y=X_kde_2D$x2, z=X_kde_2D$fhat,
        xlab="Current Eruption Duration", ylab="Next Eruption Duration")
points(data_new, col="red", pch=20)
}
...
{for(w in (0:72)*5){
  persp(x=X_kde_2D$x1, y=X_kde_2D$x2, z=X_kde_2D$fhat, phi=30,
        theta=w, col="plum",
        xlab="Current Eruption Duration", ylab="Next Eruption Duration",
        zlab="Density")
  Sys.sleep(0.5)
...
{for(h0 in h_seq){
  X_kde_2D = bkde2D(data_new, bandwidth = h0,range.x=list(c(0.5,6),c(0.5,6)))
  contour(x=X_kde_2D$x1, y=X_kde_2D$x2, z=X_kde_2D$fhat,
          xlab="Current Eruption Duration", ylab="Next Eruption Duration",
          main=paste("h =",h0))
  points(data_new, col="red", pch=20)
...
h0 = 0.1
X_kde_2D = bkde2D(data_new, bandwidth = h0,range.x=list(c(0.5,6),c(0.5,6)))
max_density = max(X_kde_2D$fhat)
  # maximum density

for(h0 in h_seq){
  X_kde_2D = bkde2D(data_new, bandwidth = h0,range.x=list(c(0.5,6),c(0.5,6)))
  persp(x=X_kde_2D$x1, y=X_kde_2D$x2, z=X_kde_2D$fhat, phi=10,
        theta=320, col="plum", zlim=c(0,max_density), main=paste("h =",h0),
        xlab="Current Eruption Duration", ylab="Next Eruption Duration",
        zlab="Density")
  Sys.sleep(0.5)
}
