## -------------------------------------------------------------
## Stat/Q Sci 403 Lab 9 | Spring 2017 | University of Washington
## -------------------------------------------------------------
###                           ###
...
plot(X,Y)
abline(lm(Y~X), lwd=3, col="purple")
  # linear fit does not give a reasonable result


data0_reg = ksmooth(x=X,y=Y, kernel = "box",bandwidth = 0.05)
...
Y = iris$Sepal.Length
plot(X,Y)
ex1_reg = ksmooth(x=X,y=Y, kernel = "normal",bandwidth = 0.5)
lines(ex1_reg, lwd=3, col="limegreen")
abline(lm(Y~X),lwd=3, col="brown")
}
...
{plot(X,Y)
ex1_reg = ksmooth(x=X,y=Y, kernel = "normal",bandwidth = 0.2)
lines(ex1_reg, lwd=3, col="dodgerblue")
ex1_reg = ksmooth(x=X,y=Y, kernel = "normal",bandwidth = 0.5)
lines(ex1_reg, lwd=3, col="limegreen")
ex1_reg = ksmooth(x=X,y=Y, kernel = "normal",bandwidth = 1)
lines(ex1_reg, lwd=3, col="purple")
legend("topleft",c("h=0.2","h=0.5","h=1"), lwd=3, 
       col=c("dodgerblue","limegreen","purple"))
}
...
lines(gr_x,gr_y, lwd=4, col="red")
lines(gr_x,gr_y+qnorm(0.999)*gr_y_sd_wild, lwd=4, col="red", lty=2)
lines(gr_x,gr_y-qnorm(0.999)*gr_y_sd_wild, lwd=4, col="red", lty=2)
...
lines(gr_x,gr_y+qnorm(0.999)*gr_y_sd_wild, lwd=2, col="limegreen", lty=2)
lines(gr_x,gr_y-qnorm(0.999)*gr_y_sd_wild, lwd=2, col="limegreen", lty=2)
lines(gr_x,gr_y+qnorm(0.999)*gr_y_sd, lwd=2, col="purple", lty=2)
lines(gr_x,gr_y-qnorm(0.999)*gr_y_sd, lwd=2, col="purple", lty=2)
legend("topleft",c("Wild Bootstrap","Empirical Bootstrap"),
       col=c("limegreen","purple"), lwd=6)
...
#### the covariate X to be 'Petal.Length' and the response Y to be 
#### 'Sepal.Length'.
#### (1) Using the smoothing bandwidth h = 0.5. Apply empirical bootstrap
####     to find a 99% CI of the regression function.
{h0 = 0.5
gr_x = seq(from=1, to =7, by=0.05)
gr_y = ksmooth(x = X,y=Y,kernel = "normal",bandwidth=h0,x.points = gr_x)$y
n = length(X)
B = 10000
gr_y_BT = matrix(NA, nrow=B, ncol=length(gr_y))
...
plot(X,Y, pch=20, cex=0.5)
lines(gr_x,gr_y, lwd=2, col="purple")
lines(gr_x,gr_y+qnorm(0.995)*gr_y_sd, lwd=2, col="purple", lty=2)
lines(gr_x,gr_y-qnorm(0.995)*gr_y_sd, lwd=2, col="purple", lty=2)
}
...
gr_y_sd_wild = sapply(1:length(gr_x), function(x){
  sd(gr_y_BT_wild[,x], na.rm=T)
})
plot(X,Y, pch=20, cex=0.5)
lines(gr_x,gr_y, lwd=2, col="red")
lines(gr_x,gr_y+qnorm(0.995)*gr_y_sd_wild, lwd=2, col="red", lty=2)
lines(gr_x,gr_y-qnorm(0.995)*gr_y_sd_wild, lwd=2, col="red", lty=2)
}
...
plot(X,Y, pch=20, col="gray50")
kernel_reg = ksmooth(x = X,y=Y,kernel = "normal",bandwidth=1)
...
{gr_x = seq(from=0,to=1, by=0.01)
gr_y_true = exp(-2*gr_x)
plot(X,Y, pch=20, col="gray")
kernel_reg = ksmooth(x = X,y=Y,kernel = "normal",bandwidth=0.1)
lines(gr_x,gr_y_true, lwd=3, col="blue")
lines(kernel_reg, lwd=3, col="red")
legend("topright",c("True","Fitted"), col=c("blue","red"), lwd=6)
}
...
h0_seq
####
{gr_x = seq(from=0,to=0.9, by=0.01)
gr_y_true = exp(-2*gr_x)
N = 10000
N_mc = 100
h0_seq = (1:20)/20*0.25
...
for(i_mc in 1:N_mc){
  ...
  X_new = rbeta(N,2,2)
  Y_new = exp(-2*X_new)+rnorm(N, sd=0.1)
  Risk_h_mc = rep(NA, length(h0_seq))
  for(i in 1:length(h0_seq)){
    h0 = h0_seq[i]
    kernel_reg = ksmooth(x = X,y=Y,kernel = "normal",bandwidth=h0,
...
#### the covariate X to be 'Petal.Length' and the response Y to be 
#### 'Sepal.Length'.
#### (1) Use the kNN regression with k=10.
####     Show the scatter plot of X and Y. Attach the fitted regression curve.
####     Also attach the regression line from a simple linear regression.
{X = iris$Petal.Length
Y = iris$Sepal.Length
plot(X,Y)
ex4_reg = knn.reg(train = X, y=Y, k = 10)
lines(X[order(X)],ex4_reg$pred[order(X)], lwd=3, col="limegreen")
abline(lm(Y~X),lwd=3, col="brown")
}
#### (2) Now fit the data with three different smoothing bandwidth 
...
####     three regression curves. 
{plot(X,Y)
ex4_reg = knn.reg(train = X, y=Y, k = 5)
lines(X[order(X)],ex4_reg$pred[order(X)], lwd=3, col="dodgerblue")
ex4_reg = knn.reg(train = X, y=Y, k = 10)
lines(X[order(X)],ex4_reg$pred[order(X)], lwd=3, col="limegreen")
ex4_reg = knn.reg(train = X, y=Y, k = 20)
lines(X[order(X)],ex4_reg$pred[order(X)], lwd=3, col="purple")
legend("topleft",c("k=5","k=10","k=20"), lwd=6, 
       col=c("dodgerblue","limegreen","purple"))
}
