## -------------------------------------------------------------
## Stat/Q Sci 403 Lab 3 | Spring 2017 | University of Washington
## -------------------------------------------------------------
###                                      ###
...
abline(h=1-exp(-1), lwd=4, col="red")
...
hist(mean_eva, probability = T, col="palegreen")
abline(v= 1-1/3, col="purple",lwd=4)
}
#### (3) Show the result using N = 50, 100, 500, 1000, ... , 1e6. 
####     Repeat the process 10 times and display the results. 
{
N_seq = c(50,100, 500,1000, 5000,10000, 5e4, 1e5, 5e5, 1e6)
plot(NULL, ylim=c(0.4, 0.9), xlim=c(50, 1e6), log="x")
abline(h=1-1/3, lwd=4, col="gray")
...
N = 1000
X_beta = rbeta(N, shape1 = 2, shape2 = 2)
eva_beta = f_target(X_beta)/dbeta(X_beta, shape1 = 2, shape2 = 2)
mean(eva_beta)
1-1/3
}
...
{
n_rep = 10000
mean_eva_beta = rep(NA, n_rep)
for(i_rep in 1:n_rep){
  X_beta = rbeta(N, shape1 = 2, shape2 = 2)
  eva_beta = f_target(X_beta)/dbeta(X_beta, shape1 = 2, shape2 = 2)
  mean_eva_beta[i_rep] = mean(eva_beta)
}
sd(mean_eva_beta)
mean_eva_uni = rep(NA, n_rep)
for(i_rep in 1:n_rep){
  X_uni = runif(N)
  eva_uni = f_target(X_uni)
  mean_eva_uni[i_rep] = mean(eva_uni)
}
sd(mean_eva_uni)
}
#### Exercise 2-2:
...
N = 1000
X_exp = rexp(N, rate = 1)
eva_exp = f_target(X_exp)/dexp(X_exp, rate=1)
mean(eva_exp)
}
...
{
n_rep = 10000
mean_eva_exp = rep(NA, n_rep)
for(i_rep in 1:n_rep){
  X_exp = rexp(N, rate = 1)
  eva_exp = f_target(X_exp)/dexp(X_exp, rate=1)
  mean_eva_exp[i_rep] = mean(eva_exp)
}
...
####     this probability.
{
hist(p0_rep, col="violet")
abline(v= 1-pnorm(1.5), lwd= 4, col="blue")
}
...
{
t.test(chickwts[chickwts$feed=="soybean",1], mu = 260)
}
...
sim_power = rep(NA, N)
for(i in 1:N){
  chickwt_sim = rnorm(n, mean=246, sd=54)
  sim_power[i] = t.test(chickwt_sim, mu=260)$p.value < alpha0
}
mean(sim_power)
}
...
plot(x=n_seq, y=sim_power_seq, ylim=c(0,1), pch=16, cex=1.5,col="blue",
     xlab="Sample size", ylab = "Power")
lines(x=n_seq, y=sim_power_seq, col="blue", lwd=3)
}



### (optional) error of histogram
### Error of histogram
