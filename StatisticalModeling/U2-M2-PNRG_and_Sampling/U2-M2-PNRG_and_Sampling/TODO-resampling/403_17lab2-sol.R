## -------------------------------------------------------------
## Stat/Q Sci 403 Lab 2 | Spring 2017 | University of Washington
## -------------------------------------------------------------
###              ###
### Part 1: CDF  ###
###              ###
...
plot(x=x_base, y=pnorm(x_base), type="l", lwd=15,
     col="blue")
...
legend("bottomright", c("Normal","T (df=1)"), lwd=3, col=c("blue","red"))
...
###  for x within [0, 4].
{
x_base = seq(from=0, to=4, by=0.01)
plot(x=x_base, y=pexp(x_base, rate=1), type="l", 
     lwd=3, col="skyblue")
lines(x=x_base, y=pexp(x_base, rate=2), lwd=3, col="dodgerblue")
lines(x=x_base, y=pexp(x_base, rate=5), lwd=3, col="blue")
lines(x=x_base, y=pexp(x_base, rate=10), lwd=3, col="midnightblue")
abline(h=0)

legend("bottomright", legend=c("Exp(1)","Exp(2)","Exp(5)","Exp(10)"), 
       col=c("skyblue","dodgerblue","blue","midnightblue"), lwd=5)
}
...
plot(x_edf, xlim=c(-5, 20), col="royalblue", lwd=5, cex=3,
     pch=4,verticals=T)
...
data2 = rexp(100, rate = 2)
data1_edf = ecdf(data1)
data2_edf = ecdf(data2)

plot(data1_edf, xlim=c(0,4), do.points=F, col="brown",
     verticals = T, lwd=2)
lines(data2_edf,  do.points=F, col="limegreen",
      verticals = T, lwd=2)
legend("bottomright", legend=c("data1","data2"), 
       col=c("brown","limegreen"), lwd=5)

ks.test(data1,data2)$p.value
}
# 2. 
{
data1 = rexp(100, rate = 1)
...
for(i in 1:10){
  ...
  data2 = qgamma(runif(100), shape=2)
...
{
data1 = rexp(100)
data2 = qexp(runif(100))
data1_edf = ecdf(data1)
data2_edf = ecdf(data2)

plot(data1_edf, do.points=F, lwd=3, verticals=T,
     col=rgb(0.5,0,1,0.9), xlim=c(0, 5))
lines(data2_edf, do.points=F, lwd=3, verticals=T,
      col=rgb(0,1,0,0.9))
legend("bottomright",c("rexp(1)","Inv CDF"), 
       col=c(rgb(0.5,0,1,0.9),rgb(0,1,0,0.9)),
       lwd=6)
}
...
# (2)
{
plot(NULL, xlim=c(0,5),ylim=c(0,1), ylab="Fn(x)", xlab="X")
for(i in 1:10){
  data1 = rexp(100)
  data2 = qexp(runif(100))
  data1_edf = ecdf(data1)
...
legend("bottomright",c("rexp(1)","Inv CDF: Exp(1)"), 
       col=c(rgb(0.5,0,1,0.9),rgb(0,1,0,0.9)),
       lwd=6)
}
...
lines(data2_edf, do.points=F, lwd=3, verticals=T,
      col=rgb(0,1,0,0.9))
legend("bottomright",c("rexp(0.5)","Inv CDF: Exp(1)"), 
       col=c(rgb(0.5,0,1,0.9),rgb(0,1,0,0.9)),
       lwd=6)
...
lines(data2_edf, do.points=F, lwd=3, verticals=T,
      col=rgb(0,1,0,0.9))
legend("bottomright",c("rexp(2)","Inv CDF: Exp(1)"), 
       col=c(rgb(0.5,0,1,0.9),rgb(0,1,0,0.9)),
       lwd=6)
...
for(i_dummy in 1:100){
  n = 1000
  plot(x=x_base, y=pnorm(x_base), col="royalblue", type="l",
       lwd=5, main=paste("Sample size:", n))
  for(i in 1:10){
    data = rnorm(n)
    data_edf = ecdf(data)
    lines(data_edf, do.points=F, lwd=2, verticals=T, 
